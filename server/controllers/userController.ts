...
export const getUserCredits = async (req: Request, res: Response) => {
  try {
    ...
    console.log(error.message, "getUserCredits error");
...
`,
    ...
    await prisma.conversation.create({
      data: {
        role: "assistant",
        content:
          "Here is the initial version of your website. You can make any changes or add new features!",
        projectId: project.id,
      },
    });
...
export const getUserProjects = async (req: Request, res: Response) => {
  try {
    ...
    console.log(error.message, "getUserProjects error");
    return res.status(500).json({ message: error.message });
  }
};

// toggle project published status controller
